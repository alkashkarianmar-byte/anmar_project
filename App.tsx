...
const App: React.FC = () => {
  ...
  const handleGetAiFeedback = async (section: AchievementSection) => {
    ...
    try {
        ...
        const prompt = `أنت مرشد تعليمي خبير ومحفز اسمه 'مرشد الذكاء الاصطناعي'. الطالب هو '${studentData.name}' في '${studentData.grade}'. أمامك إنجاز قام به الطالب. الإنجاز هو:
العنوان: ${section.title}
الوصف: ${section.content}

مهمتك هي كتابة تعليق قصير (2-3 جمل) ومباشر باللغة العربية. يجب أن يحتوي التعليق على:
1. إشادة محددة ودافئة بالإنجاز.
2. اقتراح عملي وواضح كخطوة تالية يمكن للطالب اتخاذها للبناء على هذا الإنجاز.

مثال على اقتراح جيد: 'للبناء على نجاحك في مسابقة العلوم، ما رأيك في محاولة بناء نموذج أولي بسيط للطاقة المتجددة في المنزل؟'
مثال على اقتراح سيء: 'استمر في العمل الجيد.'
